 ...
 * @param time，过期时间
 */
TLStorage.setItem = function (key, value, time) {
  try {
 ...
 * @param time
 */
TLStorage.autoDelete = function (key, time) {
  setTimeout(() => {
 ...
 */
TLStorage.wrapValue = function(value, time) {
  return {
    value: value,
 ...
 * @returns {*}
 */
TLStorage.getItem = function (key) {
 ...
 * @param key
 */
TLStorage.removeItem = function(key) {
  localStorage.removeItem(key)
}
